...
| **Putty** | CLI 환경에서 프로그래밍 가능 | **MQTT** | 라즈베리 파이 메시지 수신 |
|  | 기본적인 리눅스 명령어 사용 가능 |  | MQTT 클라이언트 제어 |
|  | 가상IP 습득 및 프로그램 운용을 위해 네트워크 방화벽 제어 가능 |  |  |
| **VncServer** | 라즈베리파이를 무선 통신을 통해 원격으로 제어 |  |  |
|  | CLI 프로그래밍 환경을 GUI 환경으로 변환 가능 |  |  |
...
| **ASP** | Set/Get 메소드를 이용해 DB와 연동하여 DB 입출력 |  **HTML** | JSON 및 AJAX를 활용해서 비동기 컨트롤러 생성  |
|  | MVC모델을 활용해서 프로그램을 체계적으로 제작 가능 |   | HTML 문서의 기본 구성 및 각 태그 컨트롤러 활용법 학습  |
|  | Bootstrap를 통해 웹 페이지 제작 |   |   |
| **DBMS** |  |**Framework** | |
| **MS-SQL** | DB 설치 및 기본 SQL 작성  | **MVC FrameWork** | MVC 패턴을 활용해서 홈페이지 작성 |
|  | 데이터베이스 다이어그램을 이용해서 전체 스키마 제어 학습 |  | AJAX 및 JSON을 활용한 정부 송수신 |
|  | DB 모델링 기법 및 정규화 | **Entity FameWork** | 스캐폴딩을 활용한 CRUD 매트릭스 구현 가능 |
